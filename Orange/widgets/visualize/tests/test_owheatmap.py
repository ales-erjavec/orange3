...
class TestOWHeatMap(WidgetTest, WidgetOutputsTestMixin):
    ...
    def test_input_data(self):
        ...
        for data in (self.data, self.housing):
            ...
            self.send_signal_wait(self.widget.Inputs.data, data)
            ...
            self.send_signal_wait(self.widget.Inputs.data, None)

        self.send_signal_wait(self.widget.Inputs.data, self.data[:0])

    def test_error_message(self):
        self.send_signal_wait(self.widget.Inputs.data, self.titanic)
        self.assertTrue(self.widget.Error.active)
        self.send_signal_wait(self.widget.Inputs.data, self.data)
        self.assertFalse(self.widget.Error.active)
    ...
    def test_information_message(self):
        ...
        self.send_signal_wait(self.widget.Inputs.data, data[:15])
        self.assertFalse(self.widget.Information.active)
        self.send_signal_wait(self.widget.Inputs.data, data[:16])
        self.assertTrue(self.widget.Information.active)
        self.assertEqual(self.widget.row_clustering, Clustering.Clustering)
        self.send_signal_wait(self.widget.Inputs.data, data[:20])
        self.assertFalse(self.widget.Information.active)
        self.send_signal_wait(self.widget.Inputs.data, data[:21])
        self.assertTrue(self.widget.Information.active)
        ...
        for kmeans_checked in (False, True):
            ...
            for col_clust in clusterings:
                ...
                for row_clust in clusterings:
                    ...
                    self.send_signal_wait(self.widget.Inputs.data, None)
                    self.send_signal_wait(self.widget.Inputs.data, self.data[0:1])
    ...
    def test_data_column_nans(self):
        ...
        self.widget.controls.merge_kmeans.setChecked(True)
        self.send_signal_wait(self.widget.Inputs.data, table)
    ...
    def test_saved_selection(self):
        ...
        self.send_signal_wait(self.widget.Inputs.data, iris)
        selected_indices = list(range(10, 31))
        self.widget.scene.widget.selectRows(selected_indices)
        self.widget.on_selection_finished()
    ...
    def test_set_split_column_key(self):
        ...
        w = self.widget
        self.send_signal_wait(self.widget.Inputs.data, data_t, widget=w)
    ...
    def test_row_color_annotations(self):
        widget = self.widget
        data = self.brown_selected[::5]
        self.send_signal_wait(widget.Inputs.data, data, widget=widget)
    ...
    def send_signal_wait(self, *args, wait=DEFAULT_TIMEOUT, **kwargs):
        return self.send_signal(*args, wait=wait, **kwargs)

    def test_col_color_annotations(self):
        widget = self.widget
        data = self._brown_selected_10()
        data_t = data.transpose(data)
        self.send_signal_wait(widget.Inputs.data, data_t, widget=widget)
        # discrete
