...
from typing import (
    ...
    Union, AnyStr, BinaryIO, Type, Mapping, Sequence, NamedTuple
)
...
from Orange.widgets.utils import (
    textimport, concurrent as qconcurrent, enum_get, unique_everseen, qname
)
...
class OWCSVFileImport(widget.OWWidget):
    ...
    def _run_import_for_new_path(self, varpath: PathItem, mime_type):
        ...
        dialect, header = default_options_for_mime_type(path, mime_type)
        options = None
...
def load_csv(path, opts, progress_callback=None):
    # type: (Union[AnyStr, BinaryIO], Options, Optional[Callable[[int, int], None]]) -> pd.DataFrame
    def dtype(coltype):
